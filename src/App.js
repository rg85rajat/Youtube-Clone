...
const URL = 'https://www.googleapis.com/youtube/v3/search?part=snippet&key=AIzaSyCNSjfWVNWnySZNo2OjpcKaGHPw81_BNMg&maxResults=7&type=video&q=';
function App() {
  ...
  useEffect(()=>{
    axios.get(`${URL}best travel places`).then(res => {
      setVideos(res.data);
      setVideoDesc(res.data.items[0]);
      setVideoList(res.data.items);
    })
